import time
import numpy as np
from loss import pseudo_labeled_graph_loss
...
def training_testing(
    ...
    paramater = sio.loadmat(
        "./pseudo_W/" + data_name + "/" + "pseudo_matrix" + str(times + 1) + ".mat"
    )
    W = paramater["Matrix"]
    W = torch.from_numpy(W)
    optimizer = torch.optim.Adam(net.parameters(), lr=0.001)
    criterion_mse, criterion_crossentropy, _, _ = net.loss()
    criterion_graphloss = pseudo_labeled_graph_loss()

    since = time.time()
    with open(data_name + "-" + network_name + "_test_cla_rate_pseudo.txt", "a") as f:
        f.write(
            "\n\n"
            + network_name
            + " r"
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if ((epoch + 1) % 500 == 0) or ((epoch + 1) == epochs):
            ...
            with torch.no_grad():
                ...
                aa = AA(C)
            with open(
                data_name + "-" + network_name + "_test_cla_rate_pseudo.txt", "a"
            ) as f:
                f.write(
                    str(epoch + 1)
                    + "  OA: "
    ...
    if not os.path.exists("./" + network_name + "_pseudo" + "/" + data_name + "/r25/"):
        os.makedirs("./" + network_name + "_pseudo" + "/" + data_name + "/r25/")
